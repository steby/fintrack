...
export async function GET(request: Request) {
  ...
  for (const household of allHouseholds) {
    ...
    // household's failure must not abort the rest of the loop, and must not leave its
    // ledger slot claimed with nothing sent.
    try {
      ...
      const html = recapEmailHtml(household.name, {
        monthName: MONTH_SHORT[target.month - 1],
        year: target.year,
        point,
      });
      for (const recipient of recipients) {
        const ok = await sendEmail({
          to: recipient.email,
          subject: `${MONTH_SHORT[target.month - 1]} ${target.year} recap — ${household.name}`,
          html,
        });
        if (ok) {
          sent++;
